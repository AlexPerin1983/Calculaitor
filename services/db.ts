import { Client, Measurement, UserInfo, Film, SavedPDF, Agendamento, Budget } from '../types';
    ...
    dbPromise = new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = (event) => {
            ...
            if (event.oldVersion < 6) {
                const measureStore = transaction!.objectStore('medidas');
                const getAllRequest = measureStore.getAll();
                getAllRequest.onsuccess = () => {
                    const allData = getAllRequest.result;
                    for (const data of allData) {
                        if (data.medidas && !data.budgets) {
                            const newBudgetData = {
                                clienteId: data.clienteId,
                                budgets: [{
                                    id: Date.now() + Math.random(),
                                    name: 'Opção 1',
                                    measurements: data.medidas,
                                    generalDiscount: { value: '', type: 'percentage' }
                                }]
                            };
                            measureStore.put(newBudgetData);
                        }
                    }
                };
            }
...
// Measurement/Budget functions
export const getBudgets = async (clientId: number): Promise<Budget[] | null> => {
    const result = await dbGet<{ clienteId: number, budgets: Budget[] }>('medidas', clientId);
    return result ? result.budgets : null;
};
export const saveBudgets = (clientId: number, budgets: Budget[]) => dbPut('medidas', { clienteId: clientId, budgets });
