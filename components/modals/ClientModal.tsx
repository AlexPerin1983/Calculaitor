...
interface ClientModalProps {
    ...
    client: Client | null;
    initialName?: string;
    onProcessImage?: (files: File[]) => Promise<Partial<Client>>;
}
...
const ClientModal: React.FC<ClientModalProps> = ({ isOpen, onClose, onSave, mode, client, initialName, onProcessImage }) => {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (!e.target.files || e.target.files.length === 0 || !onProcessImage) return;

        const files = Array.from(e.target.files);
        setIsProcessing(true);
        setError(null);
        try {
            const extractedData = await onProcessImage(files);
            setFormData(prev => ({
                ...prev,
                nome: extractedData.nome || prev.nome,
                telefone: extractedData.telefone ? applyPhoneMask(extractedData.telefone) : prev.telefone,
                cpfCnpj: extractedData.cpfCnpj ? applyCpfCnpjMask(extractedData.cpfCnpj) : prev.cpfCnpj,
                email: extractedData.email || prev.email,
                // A IA retorna um endereço completo, que colocamos no logradouro para edição manual.
                logradouro: (extractedData as any).endereco || prev.logradouro,
            }));
        } catch (err) {
    ...
    const modalTitle = (
        <div className="flex items-center justify-between w-full">
            ...
            {onProcessImage && (
                <>
                    <input type="file" ref={fileInputRef} onChange={handleImageUpload} className="hidden" accept="image/*" multiple disabled={isProcessing} />
                    <button type="button" onClick={triggerFileInput} disabled={isProcessing} className="flex-shrink-0 ml-2 text-slate-500 hover:text-slate-800 disabled:text-slate-400 disabled:cursor-wait transition-colors p-2 rounded-full hover:bg-slate-100 flex items-center gap-2" aria-label="Preencher com IA a partir de imagem">
                        {isProcessing ? <><i className="fas fa-spinner fa-spin"></i><span className="text-sm font-normal hidden sm:inline">Analisando...</span></> : <><i className="fas fa-camera"></i><span className="text-sm font-normal hidden sm:inline">Escanear</span></>}
                    </button>
                </>
            )}
        </div>
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={modalTitle}>
            <form id="clientForm" onSubmit={handleSubmit} className="space-y-4">
                 <fieldset disabled={isProcessing || isFetchingCep || isSearchingByAddress} className="space-y-4">
                ...
                <div className="pt-4">
                    <button type="submit" disabled={isProcessing || isFetchingCep || isSearchingByAddress} className="w-full p-3 bg-slate-800 text-white rounded-md hover:bg-slate-700 transition duration-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 disabled:bg-slate-500 disabled:cursor-wait">
                        {isProcessing || isFetchingCep || isSearchingByAddress ? 'Processando...' : 'Salvar Cliente'}
                    </button>
                </div>
            </form>
        </Modal>
